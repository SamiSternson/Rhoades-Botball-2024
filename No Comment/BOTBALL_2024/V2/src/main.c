#include <kipr/wombat.h> 
void STEPPER_SERVO();
int main()
{
    int astro_servo=0;
    int solar_servo=1;
    int au=2047; 
    int ad=500;
    int su=1800;
    int sd=503;
    float speed=0.46;
    int i=0;
    int angle=70;//change this angle
    create3_connect();
    create3_wait();
    create3_rotate_degrees(-90, 45);
    create3_wait();
    create3_drive_straight(0.21, speed);
    create3_wait();
    STEPPER_SERVO(astro_servo, 20, 500);
    STEPPER_SERVO(astro_servo, 20, au);
    while (create3_sensor_bump(1)==0 && create3_sensor_bump(2)==0)
    {
        create3_velocity_set_components(speed, 0);
        create3_wait();
        if (digital(9)==1)
        {
            create3_velocity_set_components(0, 0);
            create3_wait();
        }
    }
    create3_drive_straight(-0.4, speed);//change this distance
    create3_wait();
    create3_rotate_degrees(-90, 45);
    create3_wait();
    while (create3_sensor_bump(1)==0 && create3_sensor_bump(2)==0)
    {
        create3_velocity_set_components(0.1, 0);
        create3_wait();
        if (digital(9)==1)
        {
            create3_velocity_set_components(0, 0);
            create3_wait();
        }
    }
    create3_drive_straight(-0.05, -0.1);
    create3_rotate_degrees(angle, 20);
    create3_wait();
    STEPPER_SERVO(astro_servo, 20, 950);
    while (digital(9)==0)
    {
        create3_velocity_set_components(0.01, 0);
        create3_wait();
        if (digital(9)==1)
        {
            create3_velocity_set_components(0, 0);
            create3_wait();
        }
    }
    STEPPER_SERVO(astro_servo, 10, au);
    create3_drive_straight(0.01, 0.1);
    create3_wait();
    STEPPER_SERVO(astro_servo, 20, ad);
    create3_drive_straight(-0.01, -0.1);
    create3_wait();
    /*create3_rotate_degrees(90-angle, 20);
    create3_wait();
    create3_drive_straight(0.75, 0.05); //prob change this dist
    create3_wait();
    create3_rotate_degrees(90, 45);
    create3_wait();
    create3_drive_straight(0.75, 0.05); //prob change this dist
    create3_wait();
    create3_rotate_degrees(-90, 45);
    create3_wait();
    create3_drive_straight(0.5, 0.05); //prob change this dist
    create3_wait();
    create3_rotate_degrees(30, 10);
    create3_wait();
    STEPPER_SERVO(solar_servo, 30, sd);
    create3_drive_straight(1, -speed);//change this dist at school
    create3_wait();*/
    return 0;
}
void STEPPER_SERVO(int port, int speed, int final)
{
    enable_servos();
    int pos=get_servo_position(port);
    if (pos>final)
    {
        while(pos>final)
        {
            set_servo_position(port, pos-speed);
            msleep(20);
            pos=get_servo_position(port);
        }
    }
    else
    {
        while(pos<final)
        {
            set_servo_position(port, pos+speed);
            msleep(20);
            pos=get_servo_position(port);
        }
    }
    set_servo_position(port, final);
    msleep(100);
    disable_servos();

}